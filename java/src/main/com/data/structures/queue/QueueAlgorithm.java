...
public class QueueAlgorithm extends Algorithm {
    ...
    private Queue<String> queue = new QueueImpl<>(100);
    ...
    protected void printInstructions() {
        ...
        System.out.println("Press 'r' to remove an item from the queue.");
    }
    ...
    public String execute(String input, BufferedReader br) throws InvalidInputException {
        if (String.valueOf(input.charAt(0)).equals(ADD)) {
            if (input.length() <= 1) {
                throw new InvalidInputException("Press enter a string to add into the queue.");
            }
